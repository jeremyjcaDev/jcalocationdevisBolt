      ...
      <div class="form-container">
        <div class="form-section">
          ...
          <div class="form-group">
            ...
            >
              <option value="standard">Devis classique (vente)</option>
              <option value="rental_only">Location uniquement</option>
            </select>
            <small class="form-hint">
              {{ formData.quote_type === 'rental_only' ? 'Tous les produits seront en location' : 'Vente de produits sans option de location' }}
            </small>
          </div>
              ...
              >
                ...
                <div class="product-details">
                  ...
                  <div class="detail-row">
                    ...
                    <span class="value total-price">{{ formatPrice(item.price * (item.quantity || 1)) }}</span>
                  </div>
  ...
  methods: {
    ...
    async loadQuoteData() {
      try {
        ...
        this.formData = {
          quote_type: result.quote_type || 'standard',
        ...
        // Initialiser la durée globale avec la durée du premier produit en location
        const firstRentalItem = this.selectedProducts.find(item => item.is_rental && item.duration_months)
        if (firstRentalItem) {
          this.formData.global_duration_months = firstRentalItem.duration_months
        }
      } catch (err) {
